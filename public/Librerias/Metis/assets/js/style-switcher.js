...
StyleSwitcher.prototype.build = function () {
    ...
    var imgPath = "../public/Librerias/Metis/";
    ...
    var fontWhite = $('<label/>').addClass('btn btn-xs btn-default').html('White').append(fgwbtn);
	...
	var fontBlack = $('<label/>').addClass('btn btn-xs btn').html('<span style="color: white;">Black</span>').append(fgbbtn);
	
	/*Agregando Boton Azul by pool*/
	var fgbbtnblu = $('<input/>').attr({
        'type': 'radio',
        'name': 'fgcolor'
    }).val('#2980E4').on('change', function (e) {
        $this.storageManager.setFgColor('#2980E4');
        $this.showChange();
    });
	var fontazul = $('<label/>').addClass('btn btn-xs btn-primary').html('Blue').append(fgbbtnblu);
	/*Agregando Boton Azul by pool*/


    var fgBtnGroup = $('<div/>').addClass('btn-group').attr('data-toggle', 'buttons').append(fontWhite, fontBlack, fontazul);
